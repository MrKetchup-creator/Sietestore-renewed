...
export default class AdminDashboardView {
    ...
    static async loadAlertasStock() {
        try {
            const response = await fetch(`${API_BASE_URL}/api/productos/low-stock?threshold=3`);
    ...
    static async loadUltimasVentas() {
        try {
            const response = await fetch(`${API_BASE_URL}/api/ventas?limit=5`);
    ...
    static async loadTopProducts() {
        ...
        try {
            let url = `${API_BASE_URL}/api/dashboard/top-productos`;
            if (categoryId !== null) {
