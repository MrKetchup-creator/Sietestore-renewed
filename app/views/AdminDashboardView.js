...
export default class AdminDashboardView {
    ...
    static render() {
        setTimeout(() => {
            // Solo mantenemos el dibujo del gráfico; los botones ya tienen onclick
            AdminDashboardView.drawSalesChart();
        }, 0);

        const ventasMes = PosView.sales.reduce((sum, sale) => sum + sale.total, 0);
        const hoy = new Date().toLocaleDateString();
        const ventasHoy = PosView.sales
            .filter(sale => sale.date.includes(hoy))
            ...
            <div class="pos-layout">
                ...
                <main class="admin-content">
                    <div class="inventory-header">
                        ...
                        <button class="new-product-btn">+ Nuevo Producto</button>
                    </div>
                            ...
                            <tbody>
                                ${PosView.products.map(product => `
                                    <tr>
                                        ...
                                        <td class="${product.stock <= 3 ? 'stock-low' : ''}">${product.stock}</td>
                                    </tr>
                                `).join('')}
                            </tbody>
                        </table>
            ...
            <div class="pos-layout">
                ...
                <main class="admin-content">
                    ...
                    <div class="reports-tabs">
                        <button class="report-tab active" onclick="showReportTab('top')">📈 Más Vendidos</button>
                        <button class="report-tab" onclick="showReportTab('stock')">⚠ Stock Bajo</button>
                        <button class="report-tab" onclick="showReportTab('history')">🕒 Historial</button>
                    </div>
                    <div class="reports-card">
                        <div id="topProductsView">
                            ...
                            <div class="report-content">
                                <div class="top-products">
                                    ...
                                    ${topProducts.length > 0
                                        ? topProducts.map(p => `
                                            <div class="bar-row">
                                                <span>${p[0]}</span>
                                                <div class="bar"><div style="width:${Math.min(p[1] * 10, 100)}%"></div></div>
                                                <strong>${p[1]}</strong>
                                            </div>
                                        `).join('')
                                ...
                                <div class="detail-table">
                                    <h2>Detalle</h2>
                                    <table>
                                        <thead><tr><th>PRODUCTO</th><th>VARIANTE</th><th>VENDIDOS</th></tr></thead>
                                        <tbody>
                                            ${topProducts.length > 0
                                                ? topProducts.map(p => {
                                                    const info = PosView.products.find(prod => prod.name === p[0]);
                                                    return `<tr><td>${p[0]}</td><td>${info ? `${info.talla} - ${info.color}` : '-'}</td><td>${p[1]} u.</td></tr>`;
                                                }).join('')
                                                : '<tr><td colspan="3">No hay ventas registradas</td></tr>'
                                            }
                                        </tbody>
                                    </table>
                                </div>
                        ...
                        <div id="historyView" style="display:none;">
                            ...
                            <table>
                                <thead><tr><th>ID</th><th>FECHA</th><th>MÉTODO</th><th>TOTAL</th></tr></thead>
                                <tbody>
                                    ${PosView.sales.length > 0
                                        ? PosView.sales.map(s => `<tr><td>#${s.id}</td><td>${s.date}</td><td>${s.payment}</td><td>$${s.total}</td></tr>`).join('')
    ...
    // Sidebar única y consistente (igual a la del POS)
    static renderSidebar(active) {
        return `
            <aside class="admin-sidebar">
                ...
                <nav class="menu">
                    <button class="${active === 'dashboard' ? 'active' : ''}" onclick="goDashboard()">Dashboard</button>
                    <button class="${active === 'pos' ? 'active' : ''}" onclick="goPos()">Punto de Venta</button>
                    <button class="${active === 'inventory' ? 'active' : ''}" onclick="goInventory()">Inventario</button>
                    <button class="${active === 'reports' ? 'active' : ''}" onclick="goReports()">Reportes</button>
                </nav>
    ...
    // Método para dibujar el gráfico (usado en dashboard)
    static drawSalesChart() {
        ...
        sales.forEach(sale => {
            ...
            const dayIndex = last7Days.findIndex(day => day === saleDateOnly);
    ...
    // Eventos específicos para inventario (solo el botón de nuevo producto)
    ...
    static showNewProductModal() {
        ...
        document.getElementById("cancelProduct").addEventListener("click", () => modal.remove());
    ...
    static createProduct() {
        ...
        document.getElementById("app").innerHTML = AdminDashboardView.renderInventory();
        AdminDashboardView.bindInventoryEvents();
    }

    // Los métodos bindReportsEvents ya no son necesarios porque los botones usan onclick,
    // pero se mantiene vacío por si se requiere en el futuro.
